 ...
 */
  ...
  .service('snapshot', function ($firebase, firebaseRef) {
    /**
     * get a snapshot of data for manipulating
     *
     */
    return function (path, limit, offset, callback) {
      var ref = firebaseRef(path);
      if (limit) {
        ref = ref.limit(limit);
      }
      if (offset) {
        ref = ref.startAt(offset);
      }
      return ref.on('value', function(snapshot){callback(snapshot);});
    };
  });
